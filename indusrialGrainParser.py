...
pathResult = f"C:\\Users\\{os.getlogin()}\\Desktop\\Parser.xlsx"
...
def excelEntry(name, city, price, date, org, link):
        ...
        dataFrame.to_excel(pathResult, index=False) 
...
def mainAgroServer():
    ...
    while indexPage <= len(pages):
        ...
        if countOldAds >= 15:
            print(Fore.MAGENTA + "[INFO]: На сайте пошли устаревшие объявления")
            break
...
####################################################
#ИЗ-ЗА ВОЕННЫХ ДЕЙСТВИЙ САЙТ НЕ РАБОТАЕТ           #
####################################################
...
def main():
    ...
    print(f"Результат программы находится по пути {pathResult}")
